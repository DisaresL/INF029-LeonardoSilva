...
//  ----- Dados do Aluno -----
//  Nome: Leonardo bruno de Almeida Silva
//  email: disaresl@gmail.com
//  Matrícula: 20212160003
//  Semestre: 2
...
int varredura(char *texto, char c, int isCaseSensitive, int cont,  int tam){
  ...
  for (int i = 0; i < tam; i++){    
    ...
    if (verificador[i] == ("a" || "ã" || "á" || "â" || "à")){
      verificador[i] = 'a';      
    }else if (verificador[i] == ("A" || "Á" || "Ã" || "À" || "Â")){
      verificador[i] = 'A';      
      ...
      verificador[i] = 'I';      
    }else if (verificador[i] == ("o" || "õ" || "ó" || "ô")){
      verificador[i] = 'o';      
    }else if (verificador[i] == ("O" || "Ó" || "Õ" || "Ô")){
      verificador[i] = 'O';      
...
int converter(int num){
  int inv = 0, temp, pot, pot2, u = 0, x = 0;
  for (int i = 0; i <= x; i++){
    if (num >= pow(10.0,i)){
      x++;
    }
...
int buscar(int numerobase, int numerobusca){ 
  ...
  for (int i = (x-1); i >= 0; i--){
    pot = (int) pow(10.0, i);
    vbas[u] = numerobase / pot;
    numerobase = numerobase % pot;
    u++;
  }
  int y= 0, k = 0;
  for (int i = 0; i <= y; i++){
  ...
  for (int i = (y-1); i >= 0; i--){
    pot = (int) pow(10.0, i);
    vbusc[k] = numerobusca / pot;
    numerobusca = numerobusca % pot;
    k++;
  }
